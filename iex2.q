...
get_last_trade:{[syms] 
  ...
  suffix: "/1.0/tops/last?symbols=",syms;

  / Parse json response and put into table
  data:get_data[main_url;suffix];
  / show data;
  data:.j.k data;
...
get_prev_day_summary:{[sym]
  ...
  suffix: "/1.0/stock/",sym,"/previous";
  
  / Parse json response and put into table
  data: enlist .j.k get_data[main_url;suffix];
...
get_minutely_summary:{[sym;date]
  ...
  sym:string(upper sym);
  date:string(date);
  suffix: "/1.0/stock/",sym,"/chart/date/",date;
...
get_key_stats:{[sym]
  ...
  sym:string(upper sym);  
  suffix: "/1.0/stock/",sym,"/stats";
  
  / Parse json response and put into table
  data: enlist .j.k get_data[main_url;suffix];
  
  / Update data types and rename symbol column to sym
  `sym xcol `symbol xcols update latestEPSDate:"D"$latestEPSDate,  exDividendDate:"D"$exDividendDate, symbol:`$symbol from data
 }
...
get_company_financials:{[sym]
  ...
  sym:string(upper sym);
  suffix: "/1.0/stock/",sym,"/financials";
...
/ There are three types of lists: mostactive, gainers and losers
predefined_iex_lists_data:{[list]
 
  list: string(list);
  suffix: "/1.0/stock/market/list/",list;
...
\
get_ref_data:{
  suffix: "/1.0/ref-data/symbols";

  / Parse json response and put into table
  data:.j.k get_data[main_url;suffix];
  `symbol xcols update symbol:`$symbol, date:"D"$date from data
 }

get_market_list:{
  suffix: "/1.0/stock/market/list";
...
get_sector_performance:{
  suffix: "/1.0/stock/market/sector-performance";

  / Parse json response and put into table
  data:.j.k get_data[main_url;suffix];

  update  name:`$name, lastUpdated:"P"$string(convert_epoch lastUpdated) from data
 }
